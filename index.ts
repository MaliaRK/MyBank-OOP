...
console.log(
  chalk.bold.underline.bgYellowBright.magenta(
    "\n\t!---WELCOME TO MY BANK APPLICATION---!"
  )
);

const accounts: { [accountNumber: number]: Bank } = {};
...
class Bank {
  ...
  generateAccountNumber(value?: number) {
    const min = 1;
    const max = 10 ** 14;
    const accountNumber = Math.floor(Math.random() * (max - min) + min);
    console.log(
      "\nAuto generated account number: ",
      chalk.underline(accountNumber)
    );
    return accountNumber;
  }

  reActivation(value?: number): void {
    console.log(
...
class Account {
  ...
  addTransctions(type: string, amount: number): void {
    const date = new Date().toLocaleString();
    this.transactions.push({ date, type, amount });
  }

  viewTransactionHistory(value?: number): void {
...
function getAccountAndAmountFromUser(): Promise<{
...
function getAccountFromUser(): Promise<Account | undefined> {
  return inquirer
    .prompt([
      {
        message: "Enter your account number: ",
...
function transfer() {
  inquirer
    .prompt([
      {
        message:
          "Add beneficiary account number:  (Make sure you enter a valid beneficiary A/C) ",
        type: "input",
        name: "beneficiary",
        validate: (input: any) => {
          return isNaN(Number(input)) ? "Only numbers are allowed" : true;
